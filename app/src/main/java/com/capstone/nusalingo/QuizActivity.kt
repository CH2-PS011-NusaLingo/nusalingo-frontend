...
class QuizActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mQuestionsList = Quiz.getQuestions()
    ...
    private fun setQuestion() {
        ...
        if (mCurrentPosition == mQuestionsList!!.size)
            btnSubmit?.text = "Selesaikan Kuis"
        else
            btnSubmit?.text = "Kirim Jawaban"
    }
                ...
                if (mSelectedOptionPosition == 0) {
                    ...
                    when {
                        mCurrentPosition <= mQuestionsList!!.size -> {
                            ...
                            intent.putExtra(Quiz.CORRECT_ANSWERS, mCorrectAnswer)
                            intent.putExtra(Quiz.TOTAL_QUESTIONS, mQuestionsList?.size)
                    ...
                    // change the text/message of the button based on the number of question
                    if (mCurrentPosition == mQuestionsList!!.size)
                        btnSubmit?.text = "Selesaikan Quiz"
                    else
                        btnSubmit?.text = "Lanjut ke soal selanjutnya"
